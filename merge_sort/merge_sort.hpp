...
void merge_sort(std::vector<T>& vec, size_t start, size_t end)
{
    ...
    std::thread first(merge_sort<T>, std::ref(vec), start, mid);
    std::thread second(merge_sort<T>, std::ref(vec), mid + 1, end);

    first.join();
    second.join();

    
    // single-std::thread version, testified.
    // merge_sort<T>(vec, start, mid);
    // merge_sort<T>(vec, mid + 1, end); 
